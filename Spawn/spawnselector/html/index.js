$(document).ready(function(){
    ...
    $(document).on("mouseenter", ".fa-map-marker-alt", function(e){
        var th = $(this)
        $(th).css("transform", "scale(1.7,1.7)");
    });

    $(document).on("mouseleave", ".fa-map-marker-alt", function(e){
        var th = $(this)
        $(th).css("transform", "scale(1.6,1.6)");
    });
    ...
    $(document).on("click", ".fa-map-marker-alt", function(e){
        if (pos != null) {
            ...
            $.post('https://spawnselector/spawn', JSON.stringify({coords: pos}));
            $('#info').hide()
        }
    });
    ...
    $(document).on("click", ".lastspawn", function(e){
        $(".container").fadeOut(500)
        $.post('https://spawnselector/lastpos', JSON.stringify({coords: pos}));
         $('#info').hide()
    });
})